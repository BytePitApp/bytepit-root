\chapter{Arhitektura i dizajn sustava}
	...
	\newpage
		...
		Entitet \textbf{problems\_on\_competitions} sadržava informacije o zadacima koji pripadaju pojedinim natjecanjima. Ovaj entitet je u \textit{many-to-many} vezi s entitetom \textit{competitions} preko atributa \textit{competition\_id}, te također u \textit{many-to-many} vezi sa entitetom \textit{problems} preko atributa \textit{problem\_id}. Sadržava atribute: \textit{id, problem\_id, competition\_id, num\_of\_points}.
		...
		\section{Dijagram razreda}
			...
			Na sljedećim slikama, 4.10, 4.11, 4.12 i 4.13 prikazani su dijagrami razreda koji pripadaju backend dijelu aplikacije. Treba napomenuti kako ovaj tip dijagrama nije prikladan za naš tip arhitekture API-ja. Naime, ovaj tip podataka je odličan za arhitekture u kojima se dosljedno prati REST model, gdje svaka operacija u pravilu mijenja jedan resurs (entitet). S druge strane, naša arhitektura se temelji na action-based (verb-based) RPC-evima (remote procedure call), te samim time dijagrami razreda temeljeni na entitetima nemaju smisla. No ipak, s obzirom da je ovo obavezan dio dokumentacije, napravili smo pseudo-dijagram u kojem su aktori zapravo naši API routeri.  
			Na slici 4.10 prikazani su ruteri logički podijeljeni metodama određenih aktora. Tako imamo "Users Router", "Admin Router" i "Auth Router" koje sve sadrži API (glavni dio backenda aplikacije). Metode implementirane u tim klasama manipuliraju podacima iz baze i podacima o trenutnom korisniku (web token i ostali podaci u kolačiću).
		...
		\section{Dijagram komponenti}
			...
			Slika 4.16 prikazuje dijagram komponenti koji ilustrira kako su komponente organizirane međusobno, unutar sustava te kako su povezane s okolinom. Sustavu se pristupa kroz dva sučelja. 
